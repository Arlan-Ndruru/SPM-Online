            ...
            <div class="card mb-4">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0 mx-4">
                        ...
                        <a href="{{ route('dashMosqueAdd') }}" class="m-3 btn btn-success"><i class="fa fa-plus"></i>
                            Tambah</a>
                        <form action="{{ route('dashMosques') }}">
                        ...
                        <table class="table table-striped align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Masjid
                                    </th>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <div class="d-flex flex-column justify-content-center">
                                                <h6 class="mb-0 text-sm">{{ $mosque->name }}</h6>
                                                <p class="text-xs text-secondary mb-0">{{ $mosque->alamat }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">{{ $mosque->name_ketua }}</p>
                                    </td>
                                    <td class="align-middle">
                                        {{-- <button type="button"
                                            class="btn btn-outline-warning btn-lg border-0 p-2  font-weight-bold text-xs btn-tooltip"
                                            data-bs-placement="top" title="Detail Mosque" data-container="body"
                                            data-animation="true" data-bs-toggle="modal"
                                            data-bs-target="#modal-default"><span class="fa fa-eye"></span>
                                            Detail</button> --}}
                                        <a href="{{ route('dashMosqueShow', $mosque->slug) }}"
                                        ...
                                        <a href="{{ route('dashMosqueEdit', $mosque->slug) }}"
                                        ...
                                        <form action="{{route('dashMosqueDelete', $mosque->slug)}}" method="POST"
                                            class="d-inline">
                                            @method('delete')
                                            @csrf
                                            <button
